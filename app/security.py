...
def _key():return urlsafe_b64encode(hashlib.sha256(os.environ['APP_SECRET'].encode()).digest())
def encrypt(s):return Fernet(_key()).encrypt(s.encode()).decode()
def decrypt(s):return Fernet(_key()).decrypt(s.encode()).decode()
def csrf(s):
...
def login_allowed(ip):
 now=time.time();attempts[ip]=[x for x in attempts.get(ip,[]) if now-x<300];return len(attempts[ip])<10
